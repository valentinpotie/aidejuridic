...
export async function POST(request: Request): Promise<Response> {
  // ✅ Next 15 (edge): cookies() est ASYNC
  ...
  // getSession suffisant ici (tu peux passer à getUser si besoin)
  ...
  try {
    ...
    const requestBodyToOpenAI = {
      user: userId, // ✅ on envoie l’ID utilisateur
      workflow: { id: resolvedWorkflowId },
      chatkit_configuration: {
        file_upload: {
          enabled: parsedBody?.chatkit_configuration?.file_upload?.enabled ?? true,
        },
      },
    };
    ...
    const clientSecret = (upstreamJson as { client_secret?: unknown })?.client_secret ?? null;
    const expiresAfter = (upstreamJson as { expires_after?: unknown })?.expires_after ?? null;
...
function extractUpstreamError(
  ...
  // details: string | { error?: string | { message?: string } } | unknown
